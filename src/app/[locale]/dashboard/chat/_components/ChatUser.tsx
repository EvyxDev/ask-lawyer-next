...
import { getRequest } from "@/lib/apis/user-requests";
...
const ChatUser: React.FC<ChatProps> = ({ chatId }) => {
  ...
  const { data: chatRequestData, isLoading: chatisApiLoading } =
    useQuery<HireRequestResponse>({
      ...
      queryFn: () => getRequest(locale, chatId, token),
      enabled: !!chatId,
    });
    ...
    const apiMessages: Message[] =
      chatData?.success && chatData.messages
        ? chatData.messages.map((msg) => ({
            id: `api-${msg.id}`,
            sender: msg.is_sent_by_me ? "You" : "lawyer",
            content: msg.message,
  ...
  const handleSendMessage = () => {
    ...
    const messageData = {
      ...
      receiverable_id: chatRequestData?.data.accepted_by as number,
      message: newMessage,
