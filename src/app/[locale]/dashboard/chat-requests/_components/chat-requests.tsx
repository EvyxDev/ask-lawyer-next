...
import { getRequests } from "@/lib/apis/user-requests";
import { useSession } from "next-auth/react";
import { useRouter } from "next/navigation";

const ChatRequests = ({type}:{type:string}) => {
        ...
        <div className="grid lg:grid-cols-12 grid-cols-1 lg:gap-8 md:gap-6 gap-4">
          <div className="lg:col-span-9 col-span-1 h-full ">
            <div className="flex md:flex-row flex-col gap-4 justify-between w-full my-4">
              ...
              {role === 2 && (   <Button onClick={()=>router.push("/consultant-chat")} className="bg-secondary hover:bg-secondary-dark">
                <IoIosAdd />
                {t("chat-request")}
              </Button>)}
              ...
              <div className="lg:p-8 md:p-4 p-0 max-w-5xl w-full flex flex-col lg:gap-8 md:gap-6 gap-4">
                <div className="flex flex-col gap-4 items-center">
                  {request?.data.map((job: HireRequest) => (
                    <Link
                      href={`/dashboard/chat-requests/${job.id}`}
                    ...
                    >
                      ...
                      <div className="text-secondary font-[500] flex flex-col gap-1 text-lg break-all break-words">
                        <h2>{job.message}</h2>
                        <h2 >{job.summary}</h2>
                      </div>
                    </Link>
