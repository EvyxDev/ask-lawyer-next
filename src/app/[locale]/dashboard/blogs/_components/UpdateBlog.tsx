...
import { getBlog, updateBlogMethod, DeleteBlog } from "@/lib/apis/blogs";
...
const UpdateBlog = ({ id }: UpdateBlogProps) => {
  ...
  const { data: countriesData = { data: [] } } = useQuery({
  ...
  const {
    ...
    defaultValues: {
      ...
      country_id: blogData?.data.country_id?.toString() || "",
      blog_category_id: blogData?.data.blog_category_id?.toString() || "",
      img: blogData?.data.image || placeholder,
    },
  });
  ...
  useEffect(() => {
    if (blogData?.success && blogData.data) {
      ...
      setValue("country_id", blogData.data.country_id?.toString() || "");
      setValue(
        "blog_category_id",
        blogData.data.blog_category_id?.toString() || ""
      );
      if (blogData.data.image) {
        setImagePreview(blogData.data.image);
      }
    }
  }, [blogData, setValue]);
  ...
  const { mutate: updateMutate, isPending: isUpdatePending } = useMutation({
    ...
    onError: (err) => {
      toast.error(t("blog_update_failed") + ": " + err.message, {
  ...
  const { mutate: deleteMutate, isPending: isDeletePending } = useMutation({
    ...
    onError: (err) => {
      toast.error(t("blog_delete_failed") + ": " + err.message, {
        className: "!bg-red-400 !text-white !border-red-500",
      });
    },
  });
      ...
      <div className="lg:m-6 md:m-4 m-0 bg-[#FFFFFF] rounded-lg shadow-md p-4">
        ...
        >
                  ...
                  <Select onValueChange={field.onChange} value={field.value}>
                    ...
                    <SelectContent>
                      <SelectGroup>
                        ...
                        {blogsCategories.map((category: City) => (
                          <SelectItem
                            key={category.id}
          ...
          <div className="w-full flex justify-end gap-2">
            ...
            >
              {isUpdatePending ? t("submitting") : t("update_blog")}
            </Button>
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              <DialogTrigger asChild>
                <Button
                  disabled={isUpdatePending || isDeletePending}
                  className="bg-red-400 hover:bg-red-300 transition-all duration-700 h-12 text-white rounded-md shadow-md text-center text-lg font-medium"
                >
                  {t("delete_blog")}
                </Button>
              </DialogTrigger>
                ...
                <DialogFooter>
                  <div className="flex w-full gap-2 justify-center items-center">
                    ...
                    <Button
                      ...
                      disabled={isDeletePending}
                    >
                      {isDeletePending ? t("deleting") : t("delete_blog")}
                    </Button>
                  </div>
                </DialogFooter>
              </DialogContent>
