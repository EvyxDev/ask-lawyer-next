...
import { getRequestPriceOfferUser } from "@/lib/apis/requestServiceform";
import { useQuery } from "@tanstack/react-query";
import { useLocale, useTranslations } from "next-intl";
import useFormatDate from "@/hooks/useFormatDate";
import LoaderPriceRequest from "./LoaderPriceRequest";

const PriceOfferUser = ({ id }: { id: number }) => {
  const locale = useLocale();
  const { data: Lawyerdata= [] ,isLoading} = useQuery({
    queryKey: ["Offer_User", locale],
    queryFn: () => getRequestPriceOfferUser(locale, String(id)),
  });
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex items-center gap-2 ">
              ...
              <p className="text-lg text-secondary">{formatdate(request?.created_at, locale)}</p>
            </div>
          </div>
          {/* Request Details */}
          <div>
          ...
          {/* Rating Status */}
          <div>
            {!request?.is_rated && request?.status === "completed" && (
              <RatingDialog requestId={request?.id} />
            )}
          </div>
        </CardContent>
      </Card>
