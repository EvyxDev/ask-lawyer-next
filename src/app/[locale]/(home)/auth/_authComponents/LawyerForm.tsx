...
import ImageUploadFormLawyer from "./ImageUploadFormLawyer";
const LawyerForm = () => {
...
const { mutate, isPending } = useMutation({
  ...
  onSuccess: (data, variables) => {
    ...
    setImagePreviews({ img: null, photo_union_card: null });
  },
  onError: (err) => {
    if (err instanceof Error) {
      try {
        const parsedErrors = JSON.parse(err.message) as Record<string, string[]>;
    ...
    <section>
      <div className="rtl:text-right w-full flex flex-col justify-center gap-8">
        ...
        >
          ...
          {step === 3 && (
            <>
              <ImageUploadFormLawyer
                errors={errors}
                imagePreviews={imagePreviews}
                handleFileChange={handleFileChange}
                handleRemoveImage={handleRemoveImage}
                handleBack={handleBack}
                handleNext={() => handleNext(["img", "photo_union_card"])}
              />
            </>
          )}
          {/* Step 4: account type */}
