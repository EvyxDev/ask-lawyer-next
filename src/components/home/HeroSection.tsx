...
const HeroSection = () => {
  ...
  return (
    <section className="w-full relative h-screen">
      ...
      <div className="absolute lg:bottom-4 bottom-2 p-4  z-20 w-full flex justify-center items-center">
        <div className="overflow-x-auto flex items-center lg:gap-6 gap-4 scroll-px-6  scroll-smooth">
          {HeroLinks.map((link, index) => (
            <Link
              key={index}
              href={link.href}
              className="bg-secondary hover:bg-secondary-dark transition-all duration-700 2xl:w-64 md:w-56 w-48 2xl:h-48 lg:h-40 h-36 shrink-0 text-white flex flex-col gap-4 items-center justify-center rounded-lg shadow-md text-center xl:text-2xl lg:text-xl text-lg font-semibold p-4 "
            >
