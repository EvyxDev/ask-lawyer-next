...
export const getLawyers= async (rate:string="asc" ,page:number=1,limit:number=8 ,points:string="desc") => {
  try {
    const response = await fetch(`${API_URL}api/active-lawyers?rate=${rate}&page=${page}&per_page=${limit}&score_points=${points} `, {
      method: "GET",
      cache: "no-cache",
    });
...
export const getLawyer = async (id: string): Promise<LawyerApiResponse> => {
  try {
    const response = await fetch(`${API_URL}api/active-lawyers/${id}`, {
      method: "GET",
      cache: "no-cache",
    });
